...
Ext.define('AP.view.well.GatheringPipelineDeviceInfoPanel', {
    ...
    initComponent: function () {
        ...
        var gatheringPipelineCombo = Ext.create(
            'Ext.form.field.ComboBox', {
                fieldLabel: cosog.string.wellName,
                id: "gatheringPipelineDeviceComb_Id",
                labelWidth: 35,
                width: 145,
                labelAlign: 'left',
                queryMode: 'remote',
                ...
                listeners: {
                    expand: function (sm, selections) {
                        gatheringPipelineCombo.getStore().loadPage(1); // 加载井下拉框的store
                    },
                    afterRender: function (combo, o) {
                        if (gatheringPipelineCombStore.getTotalCount() > 0) {
                            var boxkey = gatheringPipelineCombStore.data.items[0].data.boxkey;
                            var boxval = gatheringPipelineCombStore.data.items[0].data.boxval;
        ...
        Ext.apply(this, {
            tbar: [gatheringPipelineCombo, '-', {
                ...
                handler: function (v, o) {
                    ...
                    var heads = "";
                    var leftOrg_Id = Ext.getCmp('leftOrg_Id').getValue();
                    var wellInformationName = Ext.getCmp('gatheringPipelineDeviceComb_Id').getValue();
...
function CreateAndLoadGatheringPipelineDeviceInfoTable(isNew) {
    ...
    var leftOrg_Id = Ext.getCmp('leftOrg_Id').getValue();
    var wellInformationName_Id = Ext.getCmp('gatheringPipelineDeviceComb_Id').getValue();
    Ext.Ajax.request({
        method: 'POST',
