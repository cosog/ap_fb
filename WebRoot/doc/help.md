...
![](../images/helpdoc/PNG/01ddbea102a9e2d5e4bed38774c09c15.png)

协议中的采控项与数据库字段映射模式：0-以地址为主(相同地址的项对应数据库同一字段)；1-以名称为准(相同名称的项对应数据库同一字段)，一般只需设置一次。
...
7、配置完成成后，启动驱动程序，启动tomcat。启动后，打开浏览器，访问并登录软件，按照1.5节说明进行操作；如需修改软件名称、界面风格，请参考1.4节。
...
![](../images/helpdoc/PNG/73eda6a159e49f7a1ead8eb000457d4f.png)
...
3）历史查询模块：通信状态统计饼图颜色(在线、离线所设背景色)。
