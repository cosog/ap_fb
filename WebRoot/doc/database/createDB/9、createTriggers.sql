...
CREATE OR REPLACE TRIGGER AP_FB.trg_a_pipelinedevice_i   after  insert or update  on tbl_pipelinedevice FOR EACH ROW
declare
...
CREATE OR REPLACE TRIGGER AP_FB.trg_a_pumpdevice_i   after  insert or update  on tbl_pumpdevice FOR EACH ROW
declare
  pump_RT   number(10);
begin
...
create or replace trigger ap_fb.trg_b_module2role_i   before  insert  on tbl_module2role FOR EACH ROW
BEGIN
...
create or replace trigger ap_fb.trg_b_pipelinealarminfo_hist_i   before  update or insert on tbl_pipelinealarminfo_hist FOR EACH ROW
declare
...
create or replace trigger ap_fb.trg_b_pipelinealarminfo_l_i   before  insert on tbl_pipelinealarminfo_latest FOR EACH ROW
BEGIN
  SELECT seq_pipelinealarminfo_latest.nextval INTO :new.id FROM dual;
end;
/

create or replace trigger ap_fb.trg_b_pipelinedevice_i   before  insert on tbl_pipelinedevice FOR EACH ROW
BEGIN
...
create or replace trigger ap_fb.trg_b_pumpalarminfo_hist_i   before  update or insert on tbl_pumpalarminfo_hist FOR EACH ROW
declare
    recordCount number(8,2) :=0;
BEGIN
...
create or replace trigger ap_fb.trg_b_pumpalarminfo_latest_i   before  insert on tbl_pumpalarminfo_latest FOR EACH ROW
BEGIN
  SELECT seq_pumpalarminfo_latest.nextval INTO :new.id FROM dual;
end;
/

create or replace trigger ap_fb.trg_b_pumpdevice_i   before  insert on tbl_pumpdevice FOR EACH ROW
BEGIN
...
create or replace trigger ap_fb.trg_b_smsdevice_i   before  insert on tbl_smsdevice FOR EACH ROW
BEGIN
