...
CREATE OR REPLACE PROCEDURE AP_FB.prd_change_auxiliarydevicename (v_oldlName    in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_change_pipelinedevicename (v_oldWellName    in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_change_pumpdevicename (v_oldWellName    in varchar2,
                                                    v_newWellName    in varchar2,
                                                    v_orgId     in varchar2) as
  wellcount number :=0;
  newwellcount number :=0;
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_change_smsdevicename (v_oldWellName    in varchar2,
                                                    v_newWellName    in varchar2,
                                                    v_orgId     in varchar2) as
  wellcount number :=0;
  newwellcount number :=0;
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_reset_sequence (sequencename IN VARCHAR2) as
  curr_val INTEGER;
BEGIN
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_clear_data is
begin
--清空所有数据
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_alarmcolor (    overviewBackgroundColor0   in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_auxiliarydevice (
  v_name in varchar2,
  v_type in number,
  v_model in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_deviceOperationLog (
  v_time in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_pipelinealarminfo (
  v_wellName in varchar2,
  v_deviceType in number,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_pipelinedevice (v_orgname   in varchar2,
                                                    v_wellName    in varchar2,
                                                    v_devicetype in NUMBER,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_pumpalarminfo (
  v_wellName in varchar2,
  v_deviceType in number,
  v_alarmTime in varchar2,
  v_itemName in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_pumpdevice (v_orgname   in varchar2,
                                                    v_wellName    in varchar2,
                                                    v_devicetype in NUMBER,
                                                    v_applicationScenariosName    in varchar2,
                                                    v_instance    in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_resourcemonitoring (
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_smsdevice (v_orgname   in varchar2,
                                                    v_wellName    in varchar2,
                                                    v_instance    in varchar2,
                                                    v_signInId    in varchar2,
                                                    v_sortNum  in NUMBER,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_save_systemLog (
  v_time in varchar2,
  v_action in number,
  v_userId in varchar2,
  v_loginIp in varchar2,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_update_auxiliarydevice (
  v_id in number,
  v_name in varchar2,
  v_type in number,
  v_model in varchar2,
  ...
  counts number :=0;
begin
  select count(1) into counts from tbl_auxiliarydevice t where t.name=v_name and t.model=v_model and t.id<>v_id;
  if counts=0 then
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_update_pipelinedevice ( v_recordId in NUMBER,
                                                    v_wellName    in varchar2,
                                                    v_devicetype in NUMBER,
                                                    v_applicationScenariosName    in varchar2,
                                                    v_instance    in varchar2,
...
begin
  --验证权限
  select count(1) into wellcount from tbl_pipelinedevice t where t.wellname=v_wellName and t.id<>v_recordId;
    if wellcount=0 then
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_update_pumpdevice ( v_recordId in NUMBER,
                                                    v_wellName    in varchar2,
                                                    v_devicetype in NUMBER,
                                                    v_applicationScenariosName    in varchar2,
                                                    v_instance    in varchar2,
...
begin
  --验证权限
  select count(1) into wellcount from tbl_pumpdevice t where t.wellname=v_wellName and t.id<>v_recordId;
    if wellcount=0 then
        if v_recordId >0 then
          Update tbl_pumpdevice t
           Set t.wellname=v_wellName,
...
CREATE OR REPLACE PROCEDURE AP_FB.prd_update_smsdevice (v_recordId   in NUMBER,
                                                    v_wellName    in varchar2,
                                                    v_instance    in varchar2,
                                                    v_signInId    in varchar2,
                                                    v_sortNum  in NUMBER) as
